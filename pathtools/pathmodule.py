...
And again by Jan Zacharias: 2012: ( __div__-operator was a must have for me)
"""

# TODO (not for me though. I don't use any of these feature, so I won't fix.)
...
expand = lambda p: os.path.normpath(os.path.expandvars(os.path.expanduser(p)))
...
class Path(_base):
    ...
    def __new__(typ, *args):
        ...
        if len(args) == 1:
            return _base.__new__(typ, expand(*args))
        return typ(expand(os.path.join(*args)))
    ...
    def __div__(self, more):
        return self.__class__(os.path.join(self, more))

    def __rdiv__(self, other):
        return self.__class__(os.path.join(other, self))
    ...
    def __mod__(self, more):
        return self.__class__(_base.__mod__(self, more))

    def __rmod__(self, other):
        return self.__class__(_base.__mod__(other, self))
    ...
    def replace(self, *args):
        return self.__class__(str.replace(self, *args))
    ...
    @property
    def non_empty(self):
        """return True if path is a file with non zero size."""
        return self.isfile() and self.size()

    @property
    def empty(self):
        """return False if path is a file with non zero size."""
        return not (self.isfile() and self.size())
    ...
    def install(self, last_is_folder=False, rights=0775):
        """mimic shells "install" command. create all folders if not excisting.
        if "last_is_folder"=False the last component is ignored. """
        left, last = map(path, self.splitpath())
        if not left.isdir():
            left.makedirs(rights)

        if not self.isdir() and last_is_folder:
            self.makedirs(rights)
        #os.chmod(self, int(rights, 8))
    # --- Modifying operations on files

    def touch(self, rights=0664):
        """ Set the access/modified times of this file to the current time.
        Create the file if it does not exist.
        """
        fd = os.open(self, os.O_WRONLY | os.O_CREAT, 0666)
        #fd = os.open(self, os.O_WRONLY | os.O_CREAT, int(rights, 8))
        try:
            os.chmod(self, rights)
        except:
            pass
        os.close(fd)
        os.utime(self, None)
